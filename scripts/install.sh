...
#
# 自定义安装目录：SYNCHRONICLE_INSTALL_DIR=~/.local/bin curl -fsSL ... | sh
# 指定版本：SYNCHRONICLE_VERSION=v1.2.3 curl -fsSL ... | sh
set -e

REPO="one-ea/SynChronicle"
BIN="synchronicle"
DEST="${SYNCHRONICLE_INSTALL_DIR:-/usr/local/bin}"
...
echo "SynChronicle 安装完成：$DEST/$BIN"
[ -n "$TAG" ] && echo "版本：$TAG"
