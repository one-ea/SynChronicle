...
export interface RuntimeObserver { reflection(event: ReflectionEvent & { agent: string }): void | Promise<void>; usage(agent: string, usage: ModelUsage | undefined): void; }
...
export class Host {
  ...
  private async run(prompt: string, label: string): Promise<void> { if (this.state === "running" || this.state === "closed") throw new Error(`host is ${this.state}`); this.state = "running"; this.runController = new AbortController(); this.emit(systemEvent(label)); try { for await (const delta of this.agent.run(prompt, this.runController.signal)) { this.runController.signal.throwIfAborted(); this.output.write(delta); await this.store.runtime.appendQueue({ seq: 0, time: new Date().toISOString(), kind: "stream_delta", priority: "background", payload: { delta } }); } this.runController.signal.throwIfAborted(); this.state = "completed"; this.emit(systemEvent("运行完成", "success")); } catch (error) { this.state = "paused"; this.emit(errorEvent(error)); throw error; } finally { this.runController = null; this.output.end(); } }
