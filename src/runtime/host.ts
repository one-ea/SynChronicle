...
import { createModelSet, type ModelSet } from "../providers/index.js";
...
export interface HostDependencies { agent?: RuntimeAgent; store?: StorePort; askUser?: AskUserHandler; persistStreamDelta?: (sequence: number, text: string) => Promise<RuntimeStreamChunk> }
...
export class Host {
  ...
  static async new(config: Config, bundle: Bundle, dependencies: HostDependencies = {}): Promise<Host> { const cfg = ConfigSchema.parse(config); const store = dependencies.store ?? new Store(cfg.output_dir ?? "output/novel"); await store.init(); const models = dependencies.agent ? createModelSet(cfg, () => ({} as ReturnType<ModelSet["forRole"]>)) : createModelSet(cfg); let runtimeAgent = dependencies.agent; let host: Host | undefined; if (!runtimeAgent) { const askUser = dependencies.askUser ?? ((questions) => host!.askUser(questions)); const built = buildCoordinator(cfg, store, models, bundle, (agent, usage, model) => host?.recordUsage(agent, normalizeUsage(usage, model)), undefined, undefined, askUser, (event) => host?.observeReflection(event), () => host?.hasBudget() ?? true); runtimeAgent = { run(prompt, signal) { const stream = built.coordinator.stream(prompt, signal); return stream.textStream; }, abort() { built.coordinator.clear(); }, close() { built.coordinator.clear(); } }; } host = new Host(cfg, runtimeAgent, store, models, dependencies.persistStreamDelta); host.usage.load(await store.usage.load()); const runMeta = await store.runMeta.load() as (RunMeta & { role_models?: Record<string, { provider: string; model: string }> }) | null; for (const [role, selection] of Object.entries(runMeta?.role_models ?? {})) { await models.swap(role, selection.provider, selection.model); host.roleModels.set(role, selection); } for (const item of await store.runtime.loadQueue()) { const payload = item.payload as RuntimeEvent | undefined; if (item.kind === "ui_event" && payload?.id) host.seenEventIds.add(payload.id); } return host; }
