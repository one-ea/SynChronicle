...
export class Host {
  ...
  async steer(instruction: string): Promise<void> { const value = instruction.trim(); if (!value) throw new Error("steer instruction is empty"); this.pendingSteer.push(value); const meta = await this.store.runMeta.load(); if (meta) await this.store.runMeta.save({ ...meta, pending_steer: [meta.pending_steer, value].filter(Boolean).join("\n"), steer_history: [...meta.steer_history, { input: value, timestamp: new Date().toISOString() }] }); }
  ...
  async latestCheckpointFingerprint(): Promise<string | null> { const databaseStore = this.store as StorePort & { latestCheckpointFingerprint?: () => Promise<string | null> }; return databaseStore.latestCheckpointFingerprint ? databaseStore.latestCheckpointFingerprint() : (await this.store.checkpoints.latestGlobal())?.digest ?? null; }
  ...
  private async run(prompt: string, label: string): Promise<void> { if (this.state === "running" || this.state === "closed") throw new Error(`host is ${this.state}`); this.state = "running"; this.runController = new AbortController(); this.emit(systemEvent(label)); try { await this.boundary("agent"); const steer = this.pendingSteer.splice(0); const effectivePrompt = steer.length ? `${prompt}\n\n用户干预意见：\n${steer.join("\n")}` : prompt; for await (const delta of this.agent.run(effectivePrompt, this.runController.signal)) { this.runController.signal.throwIfAborted(); this.output.write(delta); await this.store.runtime.appendQueue({ seq: 0, time: new Date().toISOString(), kind: "stream_delta", priority: "background", payload: { delta } }); } this.runController.signal.throwIfAborted(); await this.boundary("agent"); this.state = "completed"; this.emit(systemEvent("运行完成", "success")); } catch (error) { this.state = "paused"; this.emit(errorEvent(error)); throw error; } finally { this.runController = null; this.output.end(); } }
