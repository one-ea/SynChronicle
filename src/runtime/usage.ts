...
export class UsageTracker {
  ...
  record(agent: string, usage: ModelUsage | undefined): void { if (!usage) { this.state.missing_assistant_usage++; return; } const target = this.state.per_agent[agent] ??= empty(); for (const totals of [this.state.overall, target]) { totals.input += usage.inputTokens ?? 0; totals.output += usage.outputTokens ?? 0; totals.cache_read += usage.cachedInputTokens ?? 0; totals.cost_usd += usage.totalCost ?? 0; totals.latency_ms = (totals.latency_ms ?? 0) + (usage.latencyMs ?? 0); } this.state.updated_at = new Date().toISOString(); }
