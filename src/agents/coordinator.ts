...
export function createCoordinator(model: LanguageModelInstance, system: string, registry: ToolRegistry, options: Omit<AgentOptions, "name" | "model" | "system" | "tools"> = {}, additionalTools: readonly (keyof ToolRegistry)[] = []) {
  const tools = Object.fromEntries([...coordinatorTools, ...additionalTools].map((name) => [name, registry[name] as RegisteredTool<any>]));
