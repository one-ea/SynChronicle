...
import type { DurableSteerCommand, StorePort } from "./port.js";

const steerInboxPath = "meta/worker-steer/inbox.json";
...
export class Store implements StorePort {
  ...
  init() { return this.io.ensureDirs(["chapters", "summaries", "drafts", "reviews", "meta", "meta/worker-steer", "meta/runtime", "meta/runtime/tasks", "meta/sessions", "meta/sessions/agents"]); }
  ...
  async applySteerCommand(commandId: string, instruction: string, fallback: RunMeta): Promise<boolean> { const commands = await this.pendingSteerCommands(); if (commands.some(({ id }) => id === commandId)) return false; const next = [...commands, { id: commandId, instruction }]; const meta = await this.runMeta.load() ?? fallback; await this.io.writeJSON(steerInboxPath, next); await this.runMeta.save({ ...meta, pending_steer: steerText(next), steer_history: [...meta.steer_history, { input: instruction, timestamp: new Date().toISOString() }] }); return true; }
  async pendingSteerCommands(): Promise<DurableSteerCommand[]> { return await this.io.readJSON<DurableSteerCommand[]>(steerInboxPath) ?? []; }
  async completeSteerDelivery(commandIds: string[]): Promise<void> { const ids = new Set(commandIds); const remaining = (await this.pendingSteerCommands()).filter(({ id }) => !ids.has(id)); await this.io.writeJSON(steerInboxPath, remaining); const meta = await this.runMeta.load(); if (meta) await this.runMeta.save({ ...meta, pending_steer: steerText(remaining) }); const progress = await this.progress.load(); if (progress?.flow === "steering" && !remaining.length) await this.progress.save({ ...progress, flow: "writing" }); }
