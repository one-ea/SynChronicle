...
export class Store {
  ...
  constructor(readonly dir: string, io = new FileIO(dir)) { this.io = io; this.progress = new ProgressStore(io); this.outline = new OutlineStore(io); this.drafts = new DraftStore(io); this.summaries = new SummaryStore(io); this.runMeta = new JSONStore(io, "meta/run.json"); this.userRules = new JSONStore(io, "meta/user_rules.json"); this.signals = new SignalStore(io); this.runtime = new RuntimeStore(io); this.characters = new CharacterStore(io); this.cast = new CastStore(io); this.world = new WorldStore(io); this.checkpoints = new CheckpointStore(io); this.sessions = new SessionStore(io); this.staging = new StagedArtifactStore(io); this.usage = new UsageStore(io); this.simulation = new SimulationStore(io); this.entities = new EntityStore(io); }
