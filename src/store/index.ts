...
import { ForeshadowStore } from "./foreshadows.js";
import { SkillPackStore } from "./skillpacks.js";
import { VersionStore } from "./versions.js";
import { PrepStore } from "./prep.js";
import { EvolutionStore } from "./evolution.js";
import { AsyncLocalStorage } from "node:async_hooks";
import type { Constitution } from "../domain/constitution.js";

export class Store {
  readonly progress: ProgressStore; readonly outline: OutlineStore; readonly drafts: DraftStore; readonly summaries: SummaryStore; readonly runMeta: JSONStore<RunMeta>; readonly userRules: JSONStore<unknown>; readonly signals: SignalStore; readonly runtime: RuntimeStore; readonly characters: CharacterStore; readonly cast: CastStore; readonly world: WorldStore; readonly checkpoints: CheckpointStore; readonly sessions: SessionStore; readonly staging: StagedArtifactStore; readonly usage: UsageStore; readonly simulation: SimulationStore; readonly entities: EntityStore; readonly branches: BranchStore; readonly materials: MaterialStore; readonly constitution: JSONStore<Constitution>; readonly foreshadows: ForeshadowStore; readonly skillpacks: SkillPackStore; readonly versions: VersionStore; readonly prep: PrepStore; readonly evolution: EvolutionStore;
  private readonly io: FileIO;
  constructor(readonly dir: string, io = new FileIO(dir)) { this.io = io; this.progress = new ProgressStore(io); this.outline = new OutlineStore(io); this.drafts = new DraftStore(io); this.summaries = new SummaryStore(io); this.runMeta = new JSONStore(io, "meta/run.json"); this.userRules = new JSONStore<unknown>(io, "meta/user_rules.json"); this.signals = new SignalStore(io); this.runtime = new RuntimeStore(io); this.characters = new CharacterStore(io); this.cast = new CastStore(io); this.world = new WorldStore(io); this.checkpoints = new CheckpointStore(io); this.sessions = new SessionStore(io); this.staging = new StagedArtifactStore(io); this.usage = new UsageStore(io); this.simulation = new SimulationStore(io); this.entities = new EntityStore(io); this.branches = new BranchStore(io); this.materials = new MaterialStore(io); this.constitution = new JSONStore(io, "meta/constitution.json"); this.foreshadows = new ForeshadowStore(io); this.skillpacks = new SkillPackStore(io); this.versions = new VersionStore(io); this.prep = new PrepStore(io); this.evolution = new EvolutionStore(io); }
  init() { return this.io.ensureDirs(["chapters", "summaries", "drafts", "reviews", "meta", "meta/prep", "meta/runtime", "meta/runtime/tasks", "meta/sessions", "meta/sessions/agents"]); }
