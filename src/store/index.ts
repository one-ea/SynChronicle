...
export class Store {
  ...
  constructor(readonly dir: string) { this.io = new FileIO(dir); this.progress = new ProgressStore(new FileIO(dir)); this.outline = new OutlineStore(new FileIO(dir)); this.drafts = new DraftStore(new FileIO(dir)); this.summaries = new SummaryStore(new FileIO(dir)); this.runMeta = new JSONStore(new FileIO(dir), "meta/run.json"); this.userRules = new JSONStore(new FileIO(dir), "meta/user_rules.json"); this.signals = new SignalStore(new FileIO(dir)); this.runtime = new RuntimeStore(new FileIO(dir)); this.characters = new CharacterStore(new FileIO(dir)); this.cast = new CastStore(new FileIO(dir)); this.world = new WorldStore(new FileIO(dir)); this.checkpoints = new CheckpointStore(this.io); this.sessions = new SessionStore(new FileIO(dir)); this.staging = new StagedArtifactStore(this.io); this.usage = new UsageStore(new FileIO(dir)); this.simulation = new SimulationStore(new FileIO(dir)); }
