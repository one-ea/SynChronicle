...
const ModelInput = z.object({ provider: z.string().trim().min(1).max(100), model: z.string().trim().min(1).max(200), status: z.enum(["active", "disabled"]), inputPrice: z.number().finite().nonnegative(), outputPrice: z.number().finite().nonnegative(), credentialReference: CredentialReference, metadata: z.record(z.unknown()).optional() }).strict();
...
function publicModel(value: unknown) {
  ...
  return { ...model, credentialSource: typeof value === "object" && value && "credentialReference" in value ? platformCredentialSource(String((value as Record<string, unknown>).credentialReference)) : model.credentialSource };
}
