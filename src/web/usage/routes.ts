...
import { auditEvents, platformSettings, quotaLedger, users } from "../../db/schema/index.js";
...
export const usageRoutes: FastifyPluginAsync<{ db: Database }> = async (app, { db }) => {
  ...
  app.get("/", async (request) => {
    ...
    const convert = (row: typeof perAgentRows[number]) => ({ key: row.key, costUsd: Number(row.costUsd), inputTokens: Number(row.inputTokens), outputTokens: Number(row.outputTokens), latencyMs: Number(row.latencyMs), credentialSources: row.credentialSources, priceSources: row.priceSources, unknownPrice: row.priceSources.includes("unknown") });
    const perAgent = perAgentRows.map(convert), perModel = perModelRows.map(convert);
    return { settings: { concurrencyLimit: user?.concurrencyLimit ?? 1, adminMaxConcurrency: setting?.concurrencyLimit ?? 4, budgetUsd: user?.budgetUsd === null ? null : Number(user?.budgetUsd), balanceUsd: Number(latest?.balance ?? 0) }, perAgent, perModel };
  });
