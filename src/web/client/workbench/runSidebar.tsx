...
export function RunSidebar(props: RunSidebarProps) {
  ...
  const modelSets = props.modelConfiguration?.modelSets ?? [];
  const selectedModelSetAvailable = modelSets.some(({ id }) => id === selectedModelSetId);
  ...
  useEffect(() => {
    const models = props.modelConfiguration?.providers.find(({ provider }) => provider === selectedProvider)?.models ?? [];
    if (selectedModelValue && !models.some(({ model }) => model === selectedModelValue)) setSelectedModelValue("");
  }, [selectedProvider, selectedModelValue, props.modelConfiguration?.providers]);
  ...
  return <aside className="workbench-panel run-sidebar run-instrument-panel" aria-label="运行状态" data-collapsed={collapsed} tabIndex={-1}>
    ...
    {!collapsed && <div className="run-sidebar-body">
      ...
      <section className="run-configuration-card run-instrument-card" aria-label="运行配置">
        ...
        <form className="sidebar-form" onSubmit={(event) => void model(event)}><h3>模型切换</h3>{configuredSet && <p className="muted-copy">{configuredSet.name} · v{configuredSet.version}</p>}<label>Agent<select name="role" required defaultValue=""><option value="" disabled>请选择</option>{Object.keys(configuredSet?.agents ?? {}).map((role) => <option key={role}>{role}</option>)}</select></label><label>Provider<select name="provider" required value={selectedProvider} onChange={(event) => setSelectedProvider(event.currentTarget.value)}><option value="" disabled>请选择</option>{props.modelConfiguration?.providers.map(({ provider }) => <option key={provider}>{provider}</option>)}</select></label><label>模型<select name="model" required value={selectedModelValue} onChange={(event) => setSelectedModelValue(event.currentTarget.value)}><option value="" disabled>请选择</option>{props.modelConfiguration?.providers.find(({ provider }) => provider === selectedProvider)?.models.map(({ model }) => <option key={model} value={model}>{model}</option>)}</select></label>{(() => { const selectedModel = props.modelConfiguration?.providers.find(({ provider }) => provider === selectedProvider)?.models.find(({ model }) => model === selectedModelValue); return selectedModel?.capabilities ? <small className="model-capability-hint">上下文 {selectedModel.capabilities.contextWindow?.toLocaleString() ?? "?"} · 最高 {selectedModel.capabilities.maxOutputTokens?.toLocaleString() ?? "?"} tokens</small> : null; })()}<label>凭证<select name="credentialId" defaultValue=""><option value="">平台凭证</option>{props.modelConfiguration?.providers.find(({ provider }) => provider === selectedProvider)?.credentials.map((credential) => <option key={credential.id} value={credential.id}>{credential.label}</option>)}</select></label><button type="submit" disabled={props.controlsDisabled || pending}>切换模型</button></form>
