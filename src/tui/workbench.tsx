...
export function Workbench({ host }: { host: TuiHost }) {
  ...
  useEffect(() => { let active = true; void (async () => { try { for await (const event of host.events()) if (active) dispatch({ type: "event", event }); } catch (error) { if (active) dispatch({ type: "error", error: message(error) }); } })(); void (async () => { try { for await (const delta of host.stream(true)) if (active) dispatch({ type: "stream", delta }); } catch (error) { if (active) dispatch({ type: "error", error: message(error) }); } })(); return () => { active = false; }; }, [host]);
