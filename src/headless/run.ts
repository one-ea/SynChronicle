...
interface HeadlessHost { store: { dir: string }; startPrepared(prompt: string): Promise<void>; resume(): Promise<{ label: string | null; error?: Error }>; replayQueue(maxItems?: number): Promise<RuntimeQueueItem[]>; events(): AsyncIterable<RuntimeEvent>; stream(includeBoundaries?: boolean): AsyncIterable<string>; close(): Promise<void> }
...
async function consumeStream(stream: AsyncIterable<string>, stdout: Writable) { let content = false; for await (const delta of stream) { if (delta === RUN_END) { if (content) stdout.write("\n"); break; } else if (delta === "\u0000clear") { if (content) stdout.write("\n\n"); content = false; } else if (delta) { stdout.write(delta); content = true; } } if (content) stdout.write("\n"); }
function replay(items: RuntimeQueueItem[], stdout: Writable, stderr: Writable) { let content = false; for (const item of items) { if (item.kind === "ui_event" && item.summary) { const parsed = RuntimeEventSchema.safeParse(item.payload); const progress = parsed.success && parsed.data.type === "reflection" ? formatEvent(parsed.data) : item.summary; stderr.write(`[${formatTime(item.time)}] [${item.category ?? "SYSTEM"}] ${progress}\n`); } else if (item.kind === "stream_delta") { const payload = item.payload as { delta?: unknown } | undefined; if (typeof payload?.delta === "string") { stdout.write(payload.delta); content = true; } } else if (item.kind === "stream_clear") { if (content) stdout.write("\n\n"); content = false; } } if (content) stdout.write("\n"); }
