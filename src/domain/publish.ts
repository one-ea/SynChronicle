...
export const PublishedEntrySchema = z.object({ id: z.string(), title: z.string().min(1).max(60), authorName: z.string().default("匿名"), synopsis: z.string().max(300).default(""), tags: z.array(z.string().max(12)).max(6).default([]), visibility: Visibility.default("public"), hue: z.number().int().min(0).max(360), publishedAt: z.string(), updatedAt: z.string(), stats: z.object({ chapters: z.number().int(), words: z.number().int() }).default({ chapters: 0, words: 0 }) });
