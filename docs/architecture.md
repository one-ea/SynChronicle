...
长跑成本的第二杠杆（第一是模型选型）。完整讲解版（含代码实例与排查案例）见 `docs/prompt-cache-design.md`。三层分工：**litellm 只做协议翻译**（openai `prompt_cache_key`、anthropic `cache_control`、能力声明 `CacheCapabilities`），**agentcore 决定缓存放置与身份**，**SynChronicle 一行配置接入**（`agents/build.go`）。
