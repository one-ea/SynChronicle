...
> - **SynChronicle** —— 应用层：一行配置接入（codebot 同理）
...
绝不回头修改中段。SynChronicle 的 `novel_context` 信封就是尾部追加式设计——它每章都变，
...
**SynChronicle**（已内置）：每个 agent 配 `CacheLastMessage: "ephemeral"` +
