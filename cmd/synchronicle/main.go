...
import (
	...
	"github.com/one-ea/SynChronicle/internal/entry/headless"
	"github.com/one-ea/SynChronicle/internal/entry/tui"
	"github.com/one-ea/SynChronicle/internal/eval"
	"github.com/one-ea/SynChronicle/internal/rules"
	buildversion "github.com/one-ea/SynChronicle/internal/version"
)
...
func main() {
	...
	// 首次引导
	needsSetup, err := bootstrap.NeedsSetup(opts.ConfigPath)
	if err != nil {
		die("config: %v", err)
	}
	if needsSetup {
...
// die 统一处理致命错误退出：打印到 stderr、落盘到 ~/.synchronicle/last-error.log，
...
func runSelfUpdate(target string) error {
	...
	if !result.Updated {
		fmt.Printf("SynChronicle 已是最新版本 %s\n", result.Version)
		return nil
	}
	fmt.Printf("SynChronicle 已更新到 %s\n", result.Version)
	fmt.Printf("安装位置：%s\n", result.Path)
	return nil
}
