...
import (
	...
	"github.com/one-ea/SynChronicle/assets"
	"github.com/one-ea/SynChronicle/internal/agents"
	"github.com/one-ea/SynChronicle/internal/agents/ctxpack"
	"github.com/one-ea/SynChronicle/internal/bootstrap"
	"github.com/one-ea/SynChronicle/internal/domain"
	"github.com/one-ea/SynChronicle/internal/host/exp"
	"github.com/one-ea/SynChronicle/internal/host/flow"
	"github.com/one-ea/SynChronicle/internal/host/imp"
	"github.com/one-ea/SynChronicle/internal/host/sim"
	modelreg "github.com/one-ea/SynChronicle/internal/models"
	"github.com/one-ea/SynChronicle/internal/notify"
	"github.com/one-ea/SynChronicle/internal/rules"
	storepkg "github.com/one-ea/SynChronicle/internal/store"
	"github.com/one-ea/SynChronicle/internal/tools"
	"github.com/one-ea/SynChronicle/internal/userrules"
...
func New(cfg bootstrap.Config, bundle assets.Bundle) (*Host, error) {
	...
	if sentinel := NewBudgetSentinel(cfg.Budget,
		...
		func(level, summary string) {
			...
			h.notifier.Send(notify.Notification{Kind: "budget", Level: level, Title: "SynChronicle: 预算", Body: summary})
		},
	); sentinel != nil {
		h.budget = sentinel
		budget = sentinel
		...
		usage.SetOnMissingUsage(func() {
			...
			h.notifier.Send(notify.Notification{Kind: "budget", Level: "warn", Title: "SynChronicle: 预算", Body: blind})
		})
	}
	...
	h.pauser = NewPausePointSentinel(store,
		func(reason string) {
			...
			h.notifier.Send(notify.Notification{Kind: "pause_point", Level: "info", Title: "SynChronicle: 验收停靠点", Body: reason})
		},
		func(level, summary string) {
			h.emitEvent(Event{Time: time.Now(), Category: "SYSTEM", Summary: summary, Level: level})
			h.notifier.Send(notify.Notification{Kind: "pause_point", Level: level, Title: "SynChronicle: 验收停靠点", Body: summary})
		},
	)
	pauser = h.pauser
	...
	h.router.SetOnRepeat(func(agent, task string, n int) {
		...
		h.notifier.Send(notify.Notification{Kind: "repeat", Level: "warn", Title: "SynChronicle: 指令重复", Body: body})
	})
	...
	onGuardBlock = func(agent, reason string, n int32) {
		...
		case "escalated":
			...
			h.notifier.Send(notify.Notification{Kind: "stop_guard", Level: "warn", Title: "SynChronicle: StopGuard", Body: body})
		case "hard_stop":
...
func (h *Host) waitDone() {
	...
	if progress != nil && progress.Phase == domain.PhaseComplete {
		...
		h.notifier.Send(notify.Notification{
			Kind: "run_end", Level: "info", Title: "SynChronicle: 创作完成",
		...
		if wasRunning {
			...
			h.notifier.Send(notify.Notification{
				Kind: "run_end", Level: "warn", Title: "SynChronicle: 创作停止",
