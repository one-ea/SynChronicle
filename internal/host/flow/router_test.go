...
func TestRoute_PendingRewriteOutranksArcEndReview(t *testing.T) {
	p := writingProgress([]int{10}, domain.FlowRewriting)
	p.PendingRewrites = []int{4}
	s := State{
		Progress:      p,
		LastCompleted: 10,
		ArcBoundary: &storepkg.ArcBoundary{
	...
	got := Route(s)
	if got == nil || got.Agent != "writer" || got.Chapter != 4 || got.Task != "重写第 4 章" {
		t.Fatalf("expected pending rewrite to win, got %+v", got)
	}
}

func TestRoute_SteeringOutranksArcEndReview(t *testing.T) {
	p := writingProgress([]int{10}, domain.FlowSteering)
	s := State{
		Progress:      p,
		LastCompleted: 10,
		ArcBoundary: &storepkg.ArcBoundary{
	...
	if got := Route(s); got != nil {
		t.Fatalf("expected steering to delegate to Coordinator, got %+v", got)
	}
}
