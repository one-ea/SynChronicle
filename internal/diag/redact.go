...
type SkelEvent struct {
	...
	HostDispatch    bool       // 原文含 Host 派发标记；只保留结构信号，不保存原文
	HostDispatchKey string     // Host 派发的稳定结构哈希（agent/task），不保存原始指令文本
...
func redactMessage(agent string, m agentcore.Message) SkelEvent {
	...
	for _, b := range m.Content {
		...
		case agentcore.ContentText:
			...
			if strings.Contains(b.Text, "[Host 下达指令]") {
				ev.HostDispatch = true
				if ev.HostDispatchKey == "" {
					ev.HostDispatchKey = hostDispatchKeyHash(b.Text)
				}
			}
...
// hostDispatchKeyHash 从 Host 指令中抽取 agent/task 行生成稳定哈希。
// 只返回哈希，不把原始指令文本或 task 正文写入脱敏骨架。
func hostDispatchKeyHash(text string) string {
	agent, task := "", ""
	for _, line := range strings.Split(text, "\n") {
		...
		case "task":
			...
			if unquoted, err := strconv.Unquote(task); err == nil {
				task = unquoted
			}
		}
	}
