...
func ChapterGaps(snap *Snapshot) []Finding {
	...
	return []Finding{{
		...
		Evidence:   fmt.Sprintf("missing=[%s], completed=[%s], flow=%s", intsToStr(gaps), intsToStr(sorted), flowStr(snap.Progress)),
		Suggestion: "commit_chapter 可能中途中断。检查 meta/pending_commit.json、chapters 目录和 meta/checkpoints.jsonl，确认缺失章节是否在提交中断后未恢复。",
	}}
}
