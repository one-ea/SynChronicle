...
// TestDefaultProjectRulesDir 锁死项目级规则目录镜像全局：./.synchronicle/rules/。
func TestDefaultProjectRulesDir(t *testing.T) {
	proj := filepath.Join("/tmp", "demo-book")
	want := filepath.Join(proj, ".synchronicle", "rules")
...
// TestDefaultOptions_ScansProjectRulesFromDotSynchronicle 端到端验证：
// DefaultOptions 把 cwd 下的 ./.synchronicle/rules/ 接进 SourceProject 来源。
func TestDefaultOptions_ScansProjectRulesFromDotSynchronicle(t *testing.T) {
	proj := t.TempDir()
	t.Chdir(proj)
	rulesDir := filepath.Join(proj, ".synchronicle", "rules")
	...
	if got == nil {
		t.Fatalf("应从 ./.synchronicle/rules/ 扫到项目规则来源，得到 %+v", srcs)
	}
