...
func TestContextToolSchemaIncludesConsumer(t *testing.T) {
	tool := NewContextTool(store.NewStore(t.TempDir()), References{}, "default")
	raw, err := json.Marshal(tool.Schema())
	if err != nil {
		t.Fatalf("Marshal schema: %v", err)
	}
	for _, want := range []string{"consumer", "architect", "writer", "editor", "coordinator"} {
...
func TestResolveContextConsumer(t *testing.T) {
	tests := []struct {
		name    string
		chapter int
		raw     string
		want    ContextConsumer
		wantErr bool
	}{
		{name: "explicit editor", chapter: 12, raw: "editor", want: ContextConsumerEditor},
		{name: "legacy chapter", chapter: 12, want: ContextConsumerWriter},
		{name: "legacy planning", want: ContextConsumerArchitect},
		{name: "coordinator", raw: "coordinator", want: ContextConsumerCoordinator},
		{name: "invalid", raw: "critic", wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := resolveContextConsumer(tt.chapter, tt.raw)
			if tt.wantErr {
				if err == nil || !strings.Contains(err.Error(), "architect, writer, editor, coordinator") {
...
func TestContextToolConsumerFieldMatrix(t *testing.T) {
	tests := []struct {
		consumer  string
		chapter   int
		required  []string
		...
		{consumer: "architect", required: []string{"planning_memory", "foundation_memory", "reference_pack", "progress_status"}, forbidden: []string{"episodic_memory", "selected_memory"}},
		{consumer: "coordinator", required: []string{"working_memory", "progress_status", "foundation_status"}, forbidden: []string{"planning_memory", "foundation_memory", "episodic_memory", "selected_memory"}},
		{consumer: "writer", chapter: 1, required: []string{"working_memory", "episodic_memory", "selected_memory", "reference_pack"}, forbidden: []string{"planning_memory", "foundation_memory", "working_memory.review_facts", "working_memory.rule_violations"}},
		{consumer: "editor", chapter: 1, required: []string{"working_memory", "episodic_memory", "reference_pack"}, forbidden: []string{"planning_memory", "foundation_memory", "selected_memory"}},
	}
	for _, tt := range tests {
		t.Run(tt.consumer, func(t *testing.T) {
			payload := executeContextFixture(t, tt.chapter, tt.consumer)
			for _, key := range tt.required {
				if !hasContextPath(payload, key) {
					t.Fatalf("%s missing %s: %#v", tt.consumer, key, payload)
				}
			}
			for _, key := range tt.forbidden {
				if hasContextPath(payload, key) {
					t.Fatalf("%s leaked %s: %#v", tt.consumer, key, payload)
				}
			}
		})
	}
...
func executeContextFixture(t *testing.T, chapter int, consumer string) map[string]any {
	t.Helper()
	s := store.NewStore(t.TempDir())
	if err := s.Init(); err != nil {
		t.Fatalf("Init: %v", err)
	}
	if err := s.Outline.SavePremise("## 核心冲突\n主角必须完成试炼。\n"); err != nil {
		t.Fatalf("SavePremise: %v", err)
	}
	if err := s.Outline.SaveOutline([]domain.OutlineEntry{{Chapter: 1, Title: "试炼", CoreEvent: "主角进入试炼"}}); err != nil {
		t.Fatalf("SaveOutline: %v", err)
	}
	if err := s.Characters.Save([]domain.Character{{Name: "林砚", Role: "主角", Description: "少年修士"}}); err != nil {
		t.Fatalf("SaveCharacters: %v", err)
	}
	if err := s.World.SaveWorldRules([]domain.WorldRule{{Category: "magic", Rule: "灵气可以炼化"}}); err != nil {
		t.Fatalf("SaveWorldRules: %v", err)
	}
	if err := s.Progress.Init("test", 2); err != nil {
		t.Fatalf("InitProgress: %v", err)
	}
	if err := s.Drafts.SaveChapterPlan(domain.ChapterPlan{Chapter: 1, Title: "试炼", Goal: "通过第一关"}); err != nil {
	...
	tool := NewContextTool(s, References{ChapterGuide: "章节指南"}, "default")
	args, err := json.Marshal(map[string]any{"chapter": chapter, "consumer": consumer})
	if err != nil {
...
func hasContextPath(payload map[string]any, path string) bool {
...
func TestContextToolKeepsFullForeshadowWhenRecallNotTriggered(t *testing.T) {
	...
	if !ok || len(selected) != 0 {
		t.Fatalf("expected empty selected_memory for small foreshadow sets, got %+v", payload["selected_memory"])
	}
}
