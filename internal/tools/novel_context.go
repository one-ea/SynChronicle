...
type ContextConsumer string

const (
	ContextConsumerArchitect   ContextConsumer = "architect"
	ContextConsumerWriter      ContextConsumer = "writer"
	ContextConsumerEditor      ContextConsumer = "editor"
	ContextConsumerCoordinator ContextConsumer = "coordinator"
)

func resolveContextConsumer(chapter int, raw string) (ContextConsumer, error) {
	consumer := ContextConsumer(strings.TrimSpace(raw))
	if consumer == "" {
		if chapter > 0 {
			return ContextConsumerWriter, nil
		}
		return ContextConsumerArchitect, nil
	}
	switch consumer {
	case ContextConsumerArchitect, ContextConsumerWriter, ContextConsumerEditor, ContextConsumerCoordinator:
		return consumer, nil
	default:
		return "", fmt.Errorf("consumer must be one of architect, writer, editor, coordinator")
	}
}
...
func (t *ContextTool) Schema() map[string]any {
	return schema.Object(
		...
		schema.Property("consumer", schema.Enum("上下文消费者", "architect", "writer", "editor", "coordinator")),
	)
}
...
func (t *ContextTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	consumer, err := resolveContextConsumer(a.Chapter, a.Consumer)
	if err != nil {
		return nil, err
	}
	if (consumer == ContextConsumerWriter || consumer == ContextConsumerEditor) && a.Chapter <= 0 {
		return nil, fmt.Errorf("chapter must be > 0 for consumer %s", consumer)
	}
	...
	if consumer == ContextConsumerWriter || consumer == ContextConsumerEditor {
	...
	} else if consumer == ContextConsumerArchitect {
	...
	applyConsumerFieldMatrix(result, consumer)
	...
	result["_loading_summary"] = buildLoadingSummary(result, a.Chapter, consumer)
	return json.Marshal(result)
}

// buildLoadingSummary 从已组装的 result 中统计各项数据量，生成一行可读摘要。
func buildLoadingSummary(result map[string]any, chapter int, consumer ContextConsumer) string {
	var parts []string

	if chapter > 0 {
		parts = append(parts, fmt.Sprintf("ch=%d", chapter))
	} else {
		parts = append(parts, string(consumer))
	}
...
func applyConsumerFieldMatrix(result map[string]any, consumer ContextConsumer) {
	switch consumer {
	case ContextConsumerArchitect:
		delete(result, "episodic_memory")
		delete(result, "selected_memory")
	case ContextConsumerCoordinator:
		for _, key := range []string{"planning_memory", "foundation_memory", "episodic_memory", "selected_memory", "references", "style_rules"} {
			delete(result, key)
		}
	case ContextConsumerWriter:
		if working, ok := result["working_memory"].(map[string]any); ok {
			delete(working, "review_facts")
			delete(working, "rule_violations")
		}
	case ContextConsumerEditor:
		delete(result, "selected_memory")
		delete(result, "review_lessons")
	}
}
