...
// DefaultConfigPath 返回全局配置文件路径 ~/.synchronicle/config.json。
...
// DefaultConfigDir 返回 ~/.synchronicle 目录路径，不访问或创建文件系统。
...
// configDir 返回 ~/.synchronicle 目录路径，不存在时创建。
...
// projectConfigPath 返回项目级配置文件的相对路径 ./.synchronicle/config.json。
func projectConfigPath() string {
	return filepath.Join(configDirName, "config.json")
}

// LoadConfig 按优先级加载并合并配置：
//  1. ~/.synchronicle/config.json（全局）
//  2. ./.synchronicle/config.json（项目级覆盖）
...
func LoadConfig(flagPath string) (Config, error) {
	...
	dir, err := configDir()
	if err != nil {
		return cfg, fmt.Errorf("resolve global config directory: %w", err)
	}
	if p := filepath.Join(dir, "config.json"); p != "" {
	...
	if err != nil {
		return cfg, fmt.Errorf("项目级配置 ./.synchronicle/config.json 解析失败（请检查 JSON 语法）: %w", err)
	}
	if found {
...
// WriteStartupError 把启动期致命错误追加写入安全的错误日志并返回文件路径。
// 已存在的正式配置目录优先；配置尚未建立时使用用户缓存目录。
// 该操作为 best-effort，失败时返回空字符串。
func WriteStartupError(msg string) string {
	dir, err := startupErrorDir()
	if err != nil {
		return ""
	}
	if err := os.MkdirAll(dir, 0o755); err != nil {
...
func startupErrorDir() (string, error) {
	...
	return filepath.Join(cacheDir, "synchronicle"), nil
}
