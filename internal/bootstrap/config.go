...
import (
	...
	"github.com/one-ea/SynChronicle/internal/models"
	"github.com/one-ea/SynChronicle/internal/utils"
...
func (c *Config) ValidateBase() error {
	...
	if !ok {
		return fmt.Errorf("provider %q 未在 providers 中配置凭证；若在 ./.synchronicle/config.json 里覆盖了 provider，需同时声明 providers.%s（含 api_key/base_url），不能只改顶层 provider: %w", c.Provider, c.Provider, errs.ErrConfig)
	}
